    ...
    for tech_id, info in ttp_map.items():
        ...
        objects.append(
            _relationship(
                ...
                "uses",
                f"threat-actor--{actor_id}",
                f"attack-pattern--{ap_id}",
                now_ts,
            )
        )
...
def _threat_actor(
    ...
    sophistication = (
        "advanced" if risk >= 0.8 else "intermediate" if risk >= 0.5 else "novice"
    )
...
def _attack_pattern(
    ...
    url = (
        info.get("url")
        or f"https://attack.mitre.org/techniques/{tech_id.replace('.', '/')}/"
    )
    return {
        "type": "attack-pattern",
        "spec_version": "2.1",
