...
- 타임라인에 `결과=<오류코드>` 또는 `상태=HTTP 4xx/5xx`가 있는 작업은 성공한 행위가 아니라 실패한 시도입니다. 오류 코드에 따라 표현을 달리하세요: `AccessDenied`·`UnauthorizedOperation` 계열은 "시도했으나 거부됐다", `NoSuchBucket`·`ResourceNotFoundException` 계열은 "시도했으나 대상이 없었다", 그 외는 "시도했으나 실패했다". 실패한 작업을 "생성했다", "획득했다"로 쓰지 마세요.
