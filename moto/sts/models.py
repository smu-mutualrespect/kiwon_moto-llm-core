...
class STSBackend(BaseBackend):
    ...
    def get_caller_identity(
        ...
        if access_key_id in {
            honeypot_profile.DEFAULT_ACCESS_KEY_ID,
            honeypot_profile.PROD_ACCESS_KEY_ID,
        }:
            return (
                honeypot_profile.CALLER_USER_ID,
                honeypot_profile.IAM_USER_ARN,
                honeypot_profile.ACCOUNT_ID,
            )

        # Default values in case the request does not use valid credentials generated by moto
        partition = get_partition(region)
        user_id = honeypot_profile.CALLER_USER_ID
        arn = (
            f"arn:{partition}:iam::{honeypot_profile.ACCOUNT_ID}:"
            f"user/{honeypot_profile.IAM_USER}"
        )
        return user_id, arn, honeypot_profile.ACCOUNT_ID
